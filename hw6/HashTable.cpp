...
void HashTable::insert(string word, int index){
	...
	while(!done_inserting){
		...
		if(table[table_position].alpha_only_word != ""){
			...
			lyric_info->song_index = index;


			// places LyricInfo struct into songs vector
			table[table_position].songs->push_back(*lyric_info);
			table_count ++;
...
vector<LyricInfo>* HashTable::find(string word, vector<LyricInfo>* tether){
	 ...
	 *
	 * If it returns a NULL pointer, it then takes the
	 * tether vector and assigns it to the space in the
	 * hash table where the input search word will be
	 * hashed to once it is added.
	 */
	...
	if(found_word){
		...
		table[table_position].songs = tether;
		return NULL;
	}
